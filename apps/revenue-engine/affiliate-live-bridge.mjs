...
function checkRateLimit(req) {
  const key = rateLimitKey(req); const now = Date.now(); const windowMs = 60_000;
  ...
  current.count += 1; requestWindow.set(key, current);
...
async function openAIJson(system, user, schema) {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({ model: textModel, store: false, input: [{ role: 'system', content: [{ type: 'input_text', text: system }] }, { role: 'user', content: [{ type: 'input_text', text: user }] }], text: { format: { type: 'json_schema', name: 'mony_work_result', strict: true, schema } } })
  });
  const retryAfter = response.headers.get('retry-after'); const raw = await response.text(); let parsed = {};
  ...
  if (!response.ok) {
    ...
    if (retryAfter) error.retryAfter = retryAfter; throw error;
  }
...
async function runWork(input = {}) {
  const task = String(input.task || '').trim(); const source = String(input.source || '').trim();
