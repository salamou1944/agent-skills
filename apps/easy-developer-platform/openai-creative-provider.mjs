...
const DNA_SCHEMA = {
  ...
  required: [
    'category','type','description','brandName','printedText','logo','color','shape','components',
    'designDetails','material','background','environment','lighting','camera','composition','objects','effects','context'
  ],
};
        ...
        content: [
          {
            ...
            text: [
              purpose,
              'Analyze only what is visibly supported by the image. Never invent a brand, text, logo, color, component, material, or feature.',
              'If an immutable attribute cannot be confidently observed, return an empty string or empty array.',
              'For printed text, transcribe only clearly visible text exactly; do not paraphrase.',
              'For color, list the dominant product colors, not the background colors.',
              'For components, list physical product components that are visibly present.',
              'For designDetails, record distinctive visible construction/design details that must not change.',
  ...
  return createProviderAdapter({
    ...
    async generateCreative(instruction, input = {}) {
      ...
      const prompt = [
        ...
        'Keep the product realistic and recognizable as the exact source product.',
    ...
    async validateOutput(dna, output = {}) {
      ...
      for (const field of ['brandName','printedText','logo','color','shape','components','designDetails','material']) {
        ...
        if (actual === '' || (Array.isArray(actual) && actual.length === 0) || JSON.stringify(source) !== JSON.stringify(actual)) {
          mismatches.push({ field, expected: expected[field], actual: observed[field] });
        }
      }
      return {
        providerIntegrity: true,
        provider: `openai:${VISION_MODEL}`,
        decision: mismatches.length ? 'BLOCK' : 'PASS',
        mismatches,
        observed,
        reason: mismatches.length ? 'vision-detected-immutable-change' : 'vision-confirmed-immutable-product-integrity',
      };
    },
  });
}
...
export function openAICreativeProviderStatus() {
  return {
    ...
    provider: `openai:${IMAGE_MODEL}`,
    visionProvider: `openai:${VISION_MODEL}`,
    generationEnabled: Boolean(API_KEY),
    integrityEnabled: Boolean(API_KEY),
