...
try{
 await wait(250);
 let r=await json('/api/health');assert.equal(r.body.ok,true);assert.equal(r.body.version,'0.3.0');
 r=await json('/api/platform');assert.equal(r.body.group,'EASY Group');assert.equal(r.body.persistence,'file-backed');assert.ok(r.body.modules.includes('workspace'));assert.ok(r.body.modules.includes('deploy'));
 ...
 r=await json('/api/checks/run',{method:'POST'});assert.equal(r.body.status,'passed');assert.equal(r.body.checks.length,12);
 console.log('EASY Developer Platform v0.3 self-test: PASS');
}finally{child.kill();}
