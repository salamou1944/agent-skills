...
try{
 await wait(300);
 let r=await json('/api/health');assert.equal(r.body.ok,true);assert.equal(r.body.version,'0.5.1');
 ...
 r=await json('/api/tests/syntax',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({project})});assert.equal(r.body.decision,'pass');assert.equal(r.body.checkedFiles,1);
 ...
 r=await json('/api/builds/'+buildId+'/advance',{method:'POST'});assert.equal(r.body.status,'tested');assert.equal(r.body.gates.tests,'passed');
 ...
 r=await json('/api/tests/run',{method:'POST'});assert.equal(r.body.status,'passed');assert.equal(r.body.checks.length,14);
 ...
 console.log('EASY Developer Platform v0.5.1 self-test: PASS');
}finally{child.kill();}
