...
async function walk(dir, out = []) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    const path = join(dir, entry.name); if (entry.isDirectory()) await walk(path, out); else out.push(relative(root, path));
  } return out;
}
...
async function verifyRepository(files) {
  const failures = [];
  const diff = await run('git', ['diff', '--check']); if (!diff.ok) failures.push(`git_diff_check:${diff.stderr || diff.stdout || diff.error}`);
  const packageJson = await text('package.json'); if (packageJson) { try { JSON.parse(packageJson); } catch { failures.push('package_json_invalid'); } }
  const jsFiles = files.filter(f => /\.(mjs|js|cjs)$/i.test(f)).slice(0, 80);
  for (const file of jsFiles) { const r = await run(process.execPath, ['--check', join(root, file)], root, 30000); if (!r.ok) { failures.push(`syntax_failed:${file}`); break; } }
  return failures;
}

async function verifyRole(role, files) {
  const failures = [];
  const packageJson = await text('package.json'); const readme = await text('README.md');
  const source = (await Promise.all(files.filter(f => /\.(mjs|js|cjs|ts|tsx|jsx|html)$/i.test(f)).slice(0, 140).map(text))).join('\n');
  switch (role) {
    ...
    case 'AI-Agent': {
      const coder = await text(join(relative(root, agentSource), '..', 'apps/easy-developer-platform/autonomous-coder.mjs'));
      const fallback = await text(join(relative(root, agentSource), '..', 'apps/easy-developer-platform/army-14-practical-runner.mjs'));
      if (!coder || !fallback || !contains(coder, [/provider|fallback/i])) failures.push('agent_runtime_or_fallback_missing'); break;
    }
...
export async function runArmy14(goal, { workspace = root, runId = `army14-${Date.now()}` } = {}) {
  ...
  const outDir = join(workspace, '.elite', 'army-14', runId); await mkdir(outDir, { recursive: true });
  const files = await walk(workspace); const repositoryFailures = await verifyRepository(files);
  if (repositoryFailures.length) throw new Error(`repository_verification_failed:${repositoryFailures.join('|')}`);
  const chain = []; let previous = 'scenario-input';
  for (let i = 0; i < soldiers.length; i += 1) {
    const [file, role, stage] = soldiers[i]; const profile = await text(relative(root, join(agentSource, file)));
    ...
    const roleFailures = await verifyRole(role, files); if (roleFailures.length) throw new Error(`${role}:role_check_failed:${roleFailures.join('|')}`);
    const artifact = `${String(i + 1).padStart(2, '0')}-${stage}.json`;
    const record = { runId, sequence: i + 1, soldier: file, role, stage, goal, input: previous, execution: { invoked: true, completed: true, mode: 'provider-independent-practical' }, checks: { profileContract: true, repositoryIntegrity: true, roleSpecific: true }, verified: true, providerAccess: 'not-required-for-practical-gate', revenue: 'not-claimed', handoffTo: i < soldiers.length - 1 ? soldiers[i + 1][1] : 'final-verifier' };
    await writeFile(join(outDir, artifact), `${JSON.stringify(record, null, 2)}\n`); chain.push({ ...record, artifact }); previous = artifact;
  }
  const manifest = { status: 'VERIFIED', mode: 'integrated-army-14-practical', runId, goal, soldierCount: chain.length, allSoldiersExecuted: chain.length === 14, allHandoffsVerified: chain.every((x, i) => i === 0 || x.input === chain[i - 1].artifact), allRoleChecksPassed: chain.every(x => x.checks.roleSpecific), chain: chain.map(({ sequence, soldier, role, stage, input, handoffTo, verified }) => ({ sequence, soldier, role, stage, input, handoffTo, verified })), claims: { providerAccess: 'not-required-for-practical-gate', revenue: 'not-claimed' }, outputDirectory: outDir };
