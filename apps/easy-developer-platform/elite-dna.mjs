...
export async function buildEngineeringDNA({ root, files = [], imports = {}, goal = '' }) {
  ...
  const edges = [];
  for (const [source, targets] of Object.entries(imports || {})) for (const target of Array.isArray(targets) ? targets : []) edges.push({ source, target });
...
export function predictImpact({ dna, changedFiles = [] }) {
  const normalized = changedFiles.map(String), affected = new Set(normalized), reasons = [];
  const edges = Array.isArray(dna?.edges) ? dna.edges : [];
  let expanded = true;
  while (expanded) { expanded = false; for (const edge of edges) { const source = normalize(edge.source), target = normalize(edge.target); if (affected.has(source) && !affected.has(target)) { affected.add(target); expanded = true; reasons.push({ from: source, to: target, reason: 'import_dependency' }); } } }
  const highFanIn = (dna?.hotspots || []).filter(x => affected.has(normalize(x.path)));
  return { changedFiles: normalized, affectedFiles: [...affected].sort(), riskSignals: highFanIn.map(x => ({ path: x.path, fanIn: x.fanIn, reason: 'high_fan_in' })), predictionHash: hash(JSON.stringify({ normalized, affected: [...affected].sort(), highFanIn })) };
}
function normalize(path) { return String(path).replaceAll('\\', '/').replace(/^\.\//, ''); }

export async function recordAttempt(path, attempt) {
  const entry = { at: new Date().toISOString(), goalHash: hash(attempt.goal || ''), planHash: hash(JSON.stringify(attempt.plan || {})), failureCode: attempt.failureCode || null, failureHash: hash(attempt.failureMessage || ''), strategy: text(attempt.strategy, 500), changedFiles: (attempt.changedFiles || []).map(normalize).sort() };
...
export function createProof({ goal, result, dna, impact, tests = [], review = null, verification = null }) { const proof = { version: 1, goal, status: result?.status || 'unknown', dnaHash: dna?.dnaHash || null, predictionHash: impact?.predictionHash || null, changedFiles: result?.changedFiles || [], tests, review, verification, generatedAt: new Date().toISOString() }; return { ...proof, proofHash: hash(JSON.stringify(proof)) }; }
export function shadowDelta({ prediction, actualFiles = [] }) { const predicted = new Set(prediction?.affectedFiles || []), actual = new Set(actualFiles.map(normalize)); return { missed: [...actual].filter(x => !predicted.has(x)).sort(), falsePositives: [...predicted].filter(x => !actual.has(x)).sort(), matched: [...actual].filter(x => predicted.has(x)).sort() }; }
