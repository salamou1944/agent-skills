...
function run(cmd,args,cwd,timeout=120000){return new Promise(res=>{const p=spawn(cmd,args,{cwd,stdio:['ignore','pipe','pipe'],shell:false});let o='',e='',settled=false,overflow=false;const limit=1_000_000;const finish=r=>{if(settled)return;settled=true;clearTimeout(t);res(r)};const t=setTimeout(()=>{p.kill('SIGKILL');finish({ok:false,code:null,timeout:true,stdout:o.slice(-3000),stderr:e.slice(-3000)})},timeout);const capture=(target,data)=>{if(settled)return;target===0?o+=data:e+=data;if(o.length+e.length>limit){overflow=true;p.kill('SIGKILL');finish({ok:false,code:null,output_limit:true,stdout:o.slice(-3000),stderr:e.slice(-3000)})}};p.stdout.on('data',d=>capture(0,String(d)));p.stderr.on('data',d=>capture(1,String(d)));p.on('close',c=>finish({ok:c===0,code:c,stdout:o.slice(-3000),stderr:e.slice(-3000),output_limit:overflow}));p.on('error',x=>finish({ok:false,error:x.message,stdout:o.slice(-3000),stderr:e.slice(-3000)}))})}
