...
async function apply(dir,candidate){const paths=new Set();for(const c of candidate.changes){validate(c);if(paths.has(c.path))throw new Error('duplicate_candidate_path:'+c.path);paths.add(c.path);const target=join(dir,c.path);await mkdir(resolve(target,'..'),{recursive:true});await writeFile(target,c.content,'utf8')}}
function validateCheckCommand(t){if(!t||typeof t.cmd!=='string'||!['git',process.execPath,'node'].includes(t.cmd))throw new Error('untrusted_test_command');if(t.args!==undefined&&!Array.isArray(t.args))throw new Error('invalid_test_args');if(t.args?.some(arg=>typeof arg!=='string'))throw new Error('invalid_test_arg');if(t.cmd==='node'||t.cmd===process.execPath){const args=t.args||[];if(args.some(arg=>['-e','--eval','-p','--print','--input-type=eval'].includes(arg)||arg.startsWith('--eval=')||arg.startsWith('--print=')))throw new Error('unsafe_node_eval');if(args.length===0||args.some(arg=>arg.startsWith('-')))throw new Error('unsafe_node_invocation')}if(t.timeout!==undefined&&(!Number.isInteger(t.timeout)||t.timeout<1||t.timeout>120000))throw new Error('invalid_test_timeout')}
async function checks(dir,tests=[]){const results=[];for(const t of tests){validateCheckCommand(t);const r=await run(t.cmd,t.args||[],dir,t.timeout||120000);results.push({name:t.name,ok:r.ok,code:r.code,stdout:r.stdout.slice(-3000),stderr:r.stderr.slice(-3000)});if(!r.ok)break}return results}
