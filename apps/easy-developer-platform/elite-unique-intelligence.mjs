...
/**
 * Elite's second intelligence layer. These primitives are deterministic and evidence-first:
 * they do not pretend to know runtime facts that have not been observed.
 */
export function generateCounterfactuals({ goal, context = '', constraints = {} }) {
  const base = `${goal}\n${context}`;
  const modes = [
    ['minimal', 'smallest safe change set'],
    ['isolated', 'change behind a narrow boundary'],
    ['defensive', 'change plus explicit failure handling'],
    ['refactor', 'structural change only when required']
  ];
...
export function immuneSignature({ failureCode = '', failureMessage = '', changedFiles = [], strategy = '' }) {
  return hash(JSON.stringify({
    ...
    changedFiles: [...new Set(changedFiles.map(clean))].sort(),
...
export function immuneGate({ knownFailures = [], failureCode = '', failureMessage = '', changedFiles = [], strategy = '' }) {
  ...
  const known = new Set((knownFailures || []).map(item => typeof item === 'string' ? item : item.signature));
  return known.has(signature)
    ? { ok: false, reason: 'known_failure_signature', signature }
    : { ok: true, signature };
}
...
export function crossProjectSignal({ project, kind, value }) {
  return { ...projectScope(project), kind: String(kind), valueHash: hash(value) };
}
...
export function stopAndExplain({ result, proof = null, verification = null, required = ['verified'] }) {
  ...
  const proofOk = Boolean(proof?.proofHash);
...
export function integritySummary({ counterfactual, immune, adversarial, evolution, crossProject }) {
  return {
    version: 1,
    counterfactualHash: hash(JSON.stringify(counterfactual || {})),
    immuneSignature: immune?.signature || null,
    adversarialHash: adversarial?.challengeHash || null,
    evolutionEvent: evolution?.event || null,
    crossProjectScope: crossProject?.scope || null
  };
}
