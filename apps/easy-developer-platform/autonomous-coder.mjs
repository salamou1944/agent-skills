...
export async function execute(goal,{env=process.env}={}){const c=cfg(env),root=c.workspace,telemetryEvents=[];c.telemetry=e=>telemetryEvents.push({...e,at:new Date().toISOString()});if(c.planFile){const plan=extractJson(await readFile(c.planFile,'utf8'));return executePlan(plan,root,1)}if(!c.apiKey&&!c.githubToken&&!c.secondaryApiKey)throw new Error('llm_provider_not_configured');let lastError='',ctx=null;for(let attempt=1;attempt<=c.maxAttempts;attempt++){ctx ||= await context(root,goal);const prompt=`Goal: ${goal}\n\nCurrent repository context:\n${JSON.stringify(ctx)}\n\nPrevious verification failure (empty on first attempt): ${lastError}\n\nReturn exactly this JSON shape: {"summary":"...","changes":[]}. The changes array may be empty when the goal is already satisfied or no safe change is justified. Otherwise choose the smallest safe set of changes that advances the goal. Do not edit CI workflows or secrets. Do not commit or push.`;const plan=await ask(prompt,c);try{return {...await executePlan(plan,root,attempt),providerTelemetry:telemetryEvents}}catch(error){lastError=error.message}}throw new Error(`verification_failed:${lastError}`)}
