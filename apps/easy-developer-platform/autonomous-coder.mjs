...
function cfg(env=process.env){
  return {
    ...
    planFile:env.EASY_OPERATOR_PLAN_FILE||'',
...
async function context(root){
 const status=await run('git',['status','--short'],root);const log=await run('git',['log','-8','--oneline','--decorate'],root);
 const files=await walk(root);const priority=files.filter(f=>/^(skills\/elite-code-engineer|skills\/code-progress-supervisor|apps\/easy-developer-platform|tests?|test\/|README)/i.test(f));const ordered=[...priority,...files.filter(f=>!priority.includes(f))];
 let used=0;const selected=[];
 for(const file of ordered){if(used>=MAX_CONTEXT_BYTES)break;try{const text=await readFile(join(root,file),'utf8');if(text.length>MAX_FILE_BYTES)continue;selected.push(`\n--- ${file} ---\n${text}`);used+=text.length+file.length+10}catch{}}
 return {status:status.stdout,log:log.stdout,files:files.slice(0,1000),selected:selected.join('')};
}
...
async function requestInference(prompt,{endpoint,model,token,providerRetries=5}){
  for(let attempt=1;attempt<=providerRetries;attempt++){
    ...
    const retryAfter=Number(response.headers.get('retry-after'));const delay=Number.isFinite(retryAfter)&&retryAfter>0?Math.min(retryAfter*1000,60000):Math.min(2000*2**(attempt-1),60000);await sleep(delay);
  }
  throw new Error('provider_http_429');
}

async function ask(prompt,c){
  if(c.apiKey){try{return await requestInference(prompt,{endpoint:c.endpoint,model:c.model,token:c.apiKey,providerRetries:c.providerRetries})}catch(error){if(error.message!=='provider_http_429'||!c.githubToken)throw error}}
